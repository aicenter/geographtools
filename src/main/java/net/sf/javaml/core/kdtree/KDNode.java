...
class KDNode<V> {
	...
	protected KDNode<V> left, right;

	protected boolean deleted;

	// Method ins translated from 352.ins.c of Gonnet & Baeza-Yates
	protected static <V> KDNode<V> ins(HPoint key, V val, KDNode<V> t, int lev, int K) {
	...
	protected static <V> KDNode<V> srch(HPoint key, KDNode<V> t, int K) {
	...
	protected static <V> void rsearch(HPoint lowk, HPoint uppk, KDNode<V> t, int lev, int K, Vector<KDNode<V>> v) {
	...
	protected static <V> void nnbr(KDNode<V> kd, HPoint target, HRect hr, double max_dist_sqd, int lev, int K,
		...
		KDNode<V> nearest = (KDNode) nnl.getHighest();
		double dist_sqd;
		...
		if (HPoint.eucdist(closest, target) < Math.sqrt(max_dist_sqd)) {
			...
			KDNode<V> temp_nearest = (KDNode) nnl.getHighest();
	...
	private KDNode(HPoint key, V val) {
